...
Run `detect-provider.sh --check-url-host --provider "$PROVIDER" --host "$HOST"` before any `gh` / `glab` OP. GitHub: github.com / `*.github.com` / `*.ghe.com` or `gh auth status --hostname "$HOST"` (origin match is not enough). GitLab: `GITLAB_HOST` / `GL_HOST` or `glab auth status --hostname "$HOST"` (origin match is not enough). Refuse rather than send env tokens to an unknown host.
...
hostname), equals `glab config get host`, or `glab auth status --hostname <host>`
