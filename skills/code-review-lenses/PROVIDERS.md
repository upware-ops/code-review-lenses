...
- **bitbucket:** `curl -sf --user "${BITBUCKET_EMAIL}:${BITBUCKET_TOKEN}" "https://api.bitbucket.org/2.0/repositories/${REPO_SLUG}/pullrequests/<N>"`. Map: id→number, destination.branch.name→baseRefName, source.branch.name→headRefName, description→body. State values: "OPEN", "DECLINED"→CLOSED, "SUPERSEDED"→CLOSED, "MERGED". Stop with "Bitbucket fork PRs are not supported." when `source.repository.full_name` differs from `destination.repository.full_name`: the checkout below fetches the source branch from `origin`.
